    ...
    >
      ...
      >
        <header class="flex items-center justify-between p-6 pb-0">
          <h2 id="forgot-password-modal-title" class="text-xl font-bold text-text">Mot de passe oublié ?</h2>
          <button
            type="button"
                ...
                @if (isFieldInvalid('email')) {
                  <p class="text-error text-xs mt-1" role="alert">
                    @if (forgotPasswordForm.controls.email.errors?.['required']) {
                      L'email est requis
                    } @else if (forgotPasswordForm.controls.email.errors?.['email']) {
            ...
            <div class="text-center space-y-4">
              <div class="flex justify-center mb-4">
                <div class="bg-success/10 p-3 rounded-full">
                  <app-icon name="lucide-check" cssClass="w-6 h-6 text-success" />
                </div>
              </div>
...
export class ForgotPasswordModalView implements OnDestroy {
  ...
  readonly forgotPasswordForm = new FormGroup<ForgotPasswordModalForm>({
  ...
  isFieldInvalid(fieldName: keyof ForgotPasswordModalForm): boolean {
